...
class Challenger(models.Model):
    ...
    status = models.CharField(max_length=1, choices=STATUS_CHOICES, default="J")
    ...
    def __str__(self):
        return (
            f"{self.user.first_name} {self.user.last_name} {self.get_status_display()}"
        )
...
class Group(models.Model):
    ...
    def update_level(self):
        ...
        if members.count() == 2:
            levels = {"J": 1, "S": 2, "P": 3}
            if levels.get(members[0].challenger.status, 1) > levels.get(
                members[1].challenger.status, 1
            ):
...
class Membership(models.Model):
    ...
    role = models.CharField(
        max_length=1, choices=ROLES_CHOICES, blank=True, default="M"
    )
